...
contract BridgeTest is Test {
    ...
    function test_pause_blocksBridgeOperations() public {
        ...
        bridge.grantRoles(guardian, 1 << 0); // GUARDIAN_ROLE = 1 << 0
    ...
    function test_pause_blocksRelayMessages() public {
        // Pause the bridge using owner's pause function
        vm.prank(initialOwner);
        bridge.pause();
        
        assertTrue(bridge.paused(), "Bridge should be paused");
        
        // Test relayMessages reverts when paused
        IncomingMessage[] memory messages = new IncomingMessage[](1);
        messages[0] = IncomingMessage({
            nonce: 0,
            sender: TEST_SENDER,
