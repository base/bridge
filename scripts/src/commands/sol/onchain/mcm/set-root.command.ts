...
import { argsSchema, handleSetRoot } from "./set-root.handler";
...
export const setRootCommand = new Command("set-root")
  .description("Set a new Merkle root for MCM operations from a proposal file")
  .option(
  ...
  .option(
    "--proposal-file <path>",
    "Path to proposal JSON file (required, contains all necessary data)"
  )
  .action(async (options) => {
    ...
    await handleSetRoot(parsed.data);
  });
