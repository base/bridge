...
import { getFinalizeSignaturesInstruction } from "@xenoliss/mcm-sol-client";
...
export async function handleFinalizeSignatures(args: Args): Promise<void> {
  try {
    logger.info("--- MCM Finalize Signatures script ---");
    ...
    // Create finalize signatures instruction
    const finalizeSignaturesIx = getFinalizeSignaturesInstruction(
      {
        // Accounts
    ...
    logger.info("Sending finalize signatures transaction...");
    const signature = await buildAndSendTransaction(
      args.deployEnv,
      [finalizeSignaturesIx],
      payer
    );

    logger.success("MCM signatures finalized successfully!");
    logger.success(
      `Transaction: https://explorer.solana.com/tx/${signature}?cluster=devnet`
    );
  } catch (error) {
    logger.error("MCM finalize signatures failed:", error);
    throw error;
  }
}
